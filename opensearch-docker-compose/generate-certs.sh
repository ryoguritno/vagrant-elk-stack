...
echo "DEBUG: Executing openssl command for os-dashboards:" >>/home/vagrant/opensearch-docker-compose/generate-certs-debug.log
echo "openssl x509 -req -extfile <(printf \"subjectAltName=DNS:os-dashboards,DNS:localhost\") -in certs/os-dashboards/os-dashboards.csr -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out certs/os-dashboards/os-dashboards.pem" >>/home/vagrant/opensearch-docker-compose/generate-certs-debug.log
openssl x509 -req -extfile <(printf "subjectAltName=DNS:os-dashboards,DNS:localhost") -in certs/os-dashboards/os-dashboards.csr -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out certs/os-dashboards/os-dashboards.pem
echo "DEBUG: openssl exit code: $?" >>/home/vagrant/opensearch-docker-compose/generate-certs-debug.log
